...
There are a huge number of very talented energy forecasters out there. But I believe they are not able to be as effective as they could be. It's not their fault. The problem is a lack of infrastructure. One of the main inputs to energy forecasts is numerical weather predictions (NWPs). These datasets can be _huge_ (petabytes per year). The aim of my work is to make it as easy as possible to work with these huge NWP datasets. This should enable all those talented energy forecasters to do their best work!
...
## Energy forecasting hasn't evolved
The rapid evolution we've seen in computer vision simply hasn't happened in the world of energy forecasting. There's little sense of a year-on-year improvement in energy forecasting. This is - frankly - a terrible state to be in, and it needs to change. We're wasting money, carbon, and time.

What's holding us back? There are, in my opinion, three main technical problems: 
1. NWP datasets are huge (petabytes per year) and very time consuming to work with. 
2. We don't have a public, common validation dataset (like ImageNet) so we can't compare forecasting approaches. (Crucially, this dataset has to be large enough to enable researchers to approach the performance that for-profit companies currently obtain. More on that point in a moment.)
